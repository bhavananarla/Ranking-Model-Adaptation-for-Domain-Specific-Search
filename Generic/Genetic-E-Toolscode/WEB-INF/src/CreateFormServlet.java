...
{
      i=Integer.parseInt(req.getParameter("num_of_emp"));
		//int 
			HttpSession session=req.getSession(true);
			//HashMap hash = new HashMap();
			session.setAttribute("FormId",req.getParameter("listid"));
			session.setAttribute("FormName",req.getParameter("listname"));
			//int i = Integer.parseInt(req.getParameter("num_of_fields"));
			session.setAttribute("num_of_fields",req.getParameter("num_of_emp"));
			int limit =Integer.parseInt(req.getParameter("num_of_emp"));// checkIt(0,i);
...
public StringBuffer commonLoop()
{
		...
		out.append("detailspage</title>");
		out.append("<script language=\"javascript\">function check(){");
		out.append(" var input =document.all.tags(\"input\");");
		out.append("for(int i=0;i<input.length;i++){  ");
		out.append("if(input[i].value==\"\"){ alert(\"please fill all fileds\"); input[i].focus();");
        out.append("return false;}");
		out.append("</script></head>");
		return out;
}
...
{
		StringBuffer out = new StringBuffer();
		out.append("<body bgcolor=#ffddcc><form name=\"form1\" action =\"CreateFormServlet\" onsubmit=\"return check()\">");
		return out;
}
...
public StringBuffer forEndLoop()
{
		...
		out.append("<body bgcolor=227050><form name=\"form1\" action =\"TestFormServlet\" onsubmit=\"return check()\">");
		return out;
}
...
public void doPost(HttpServletRequest req,HttpServletResponse res) throws ServletException,IOException
{
		...
		*/
		...
		//String nexttosubmit="true";
		    ...
		    //  System.out.println("this is my value"+abc);
		...
		if(req.getParameter("enter").equals("true"))		
		{
			...
			cond=i;//req.getParameter("num_of_fields");//Integer.parseInt((String)session.getAttribute("num_of_fields"));
